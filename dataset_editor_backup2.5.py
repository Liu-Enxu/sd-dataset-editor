...
-check for change of files still need to change (destroy_frames now inside update)
    ...
    class LinkedImages:
        def __init__(self,imgsize,img,filepath,app):
            ...
            if filepath:
                ...
                # self.app.overview_sub_frames.append(self.frameCard)
        ...
        if self.filepath:
            ...
            for _type in filetype:
                for picfilepath in glob.glob(self.filepath + _type):
                    # if not (curr and picfilepath == curr.filepath):
                    # img
                    imgsize = int(self.overview_frame_width-self.border*1.5)

                    # son - FrameCards
                                    # new_frame = self.FrameCards(imgsize,img,picfilepath,self) # img needs to be a class
                                    # new_frame.pack(side=tk.TOP, anchor=tk.CENTER, expand=True)
                                    # self.overview_sub_frames.append(new_frame) # need to delete? # extend add pixels, use append

                                    # father - LindedImages
                                    # todo: directly open img or pass in var?
                    new_LinkedImg = self.LinkedImages(imgsize,Image.open(picfilepath),picfilepath,self)

                    # if self.photos:
                                    #     # set prev & next of linkedlist
                                    #     list(self.photos.keys())[-1].next = new_LinkedImg
                                    #     new_LinkedImg.prev = list(self.photos.keys())[-1]
                                    # else:
                                    #     self.DummyHead.next = new_LinkedImg
                                    #     new_LinkedImg.prev = self.DummyHead
                                    # self.photos[new_LinkedImg] = new_frame
                    ...
                    self.LinkedListEnd = new_LinkedImg
                        ...
                        # curr = curr
                    # else:
                        # self.LinkedListEnd = curr
                        # pass

                    # curr = curr.next if curr else None

            # if curr:
            #     self.LinkedListEnd.next =None
